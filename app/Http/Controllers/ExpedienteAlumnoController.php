...
class ExpedienteAlumnoController extends Controller
{
    ...
    public function registrarInscripcion(Request $request, $id_alumno)
    {
        ...
        $validator = Validator::make($request->all(), [
            'programa_id'    => 'required|integer|exists:programas_predefinidos,id_programa',
        ...
        // Ajusta a tu lógica real de inscripción si usas más campos
        $inscripcion = DB::table('inscripciones')->insert([
            'id_alumno'    => $id_alumno,
            'id_programa'  => $request->programa_id,
    ...
    public function agregarPrograma(Request $request)
    {
        $validator = Validator::make($request->all(), [
            ...
            'id_programa' => 'required|integer|exists:programas_predefinidos,id_programa',
        ]);
        ...
        if ($alumno && $alumno->status == 1) {
            ...
            if ($programa && $programa->status == 1) {
                // --- BLOQUE DE LÓGICA LEGACY PARA AGREGAR PROGRAMA ---
                $mes = date("m");
                $anio = date("Y");
                $dia = date("d");
    ...
    public function getAdeudosPorAlumno($id)
    {
        $total = 0;
        $data = [];
        $adeudosProgramas = DB::table('adeudos_programas')->where('id_alumno', $id)->orderBy('fecha_limite')->get();
        $adeudosSecundarios = DB::table('adeudos_secundarios')->where('id_alumno', $id)->orderBy('concepto')->get();
        ...
        if ($adeudosProgramas->isEmpty() && $adeudosSecundarios->isEmpty()) {
            ...
            foreach ($adeudosProgramas as $adeudo) {
                ...
                if ($adeudo->id_programa != 000) {
                    $nombre_programa = DB::table('programas_predefinidos')->where('id_programa', $adeudo->id_programa)->value('nombre');
                } else {
    ...
    // ========== 5. CLASES/PROGRAMAS POR ALUMNO ==========
    public function getProgramasPorAlumno($id)
    {
        $programasCount = DB::table('registro_predefinido')->where('id_alumno', $id)->count();
    ...
    // ========== 7. PROGRAMAS DISPONIBLES PARA INSCRIBIRSE ==========
    public function obtenerProgramas($id)
    {
        $programas = DB::table('programas_predefinidos')->where('ocultar', 0)->orderBy('nombre')->get();
    ...
    $validator = Validator::make($request->all(), [
        // Si en legacy hay más campos, agrega aquí
    ]);
    ...
    // Obtener el precio de recargo de la tabla 'informacion'
    $informacion = DB::table('informacion')->first();
    ...
    return response()->json(['message' => 'Recargo registrado exitosamente'], 200);
    }
    ...
    if ($alumno->status == 1) {
        ...
        $mes_str = strtoupper($meses[$mes]);
        $fecha = strtoupper("$dia/$mes_str/$ano");
        $periodo = $fecha;
        $fecha_db = date("Y-m-d");
    ...
    $meses = ['Jan' => 'ENE', 'Apr' => 'ABR', 'Aug' => 'AGO', 'Dec' => 'DIC'];
    $mes_str = strtoupper($meses[$mes] ?? $mes);
    $fechaDia = "$dia/$mes_str/$ano";
    ...
    // Generación de PDF con la librería DOMPDF (si tienes el view 'recibo')
    $result = [
        'id_alumno' => $id_alumno,
    ...
    // ========== 11. ELIMINAR CLASE (puedes dejarlo vacío si no lo usas aún) ==========
    public function eliminarClase(Request $request)
    {
    // Método disponible, implementa tu lógica si lo necesitas.
    return response()->json(['message' => 'Método eliminarClase disponible.']);
    }
